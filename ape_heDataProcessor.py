    ...
    def extract_keys_from_myDict(myDict, parent_key=None): 
        ...
        for key, value in myDict.items():
            ...
            if isinstance(value, dict):
                for el in APE_HE_DataProcessor.extract_keys_from_myDict(value, new_key):
                    if el[-1] not in APE_HE_DataProcessor.avoidFields:
                        keys_list.append(el)
                    else:
                ...
                for i, item in enumerate(value):
                    if isinstance(item, dict):
                        for el in APE_HE_DataProcessor.extract_keys_from_myDict(item, new_key):
                            if el[-1] not in APE_HE_DataProcessor.avoidFields:
                                keys_list.append(el)
                            else:
                                keys_list.append(el[:-1])
    ...
    def map_equivalencies(metadata_dict, keys_path_schema, equivalencies):
        ...
        for key in keys_path_schema:
            ...
            if matched_key in keys_metadata and matched_key in metadata_dict:
                ...
                APE_HE_DataProcessor.set_nested_value(mapped_metadata_dict, key, value)

        return mapped_metadata_dict


    @staticmethod
    def populate_schema(mySchema_dict, mapped_metadata_dict):
        """
        ...
        """
        mappedDict_keys = APE_HE_DataProcessor.extract_keys_from_myDict(mapped_metadata_dict)
        ...
        for key_path in mappedDict_keys:
            ...
            try:
                ...
                if last_key in meta_ref:
                    APE_HE_DataProcessor._process_schema_values(sche_ref, meta_ref, key_path, last_key)
    ...
    def _process_schema_values(sche_ref, meta_ref, key_path, last_key):
        try:
            if last_key in meta_ref:
                ...
                logging.warning(f"Missing key in metadata: {last_key}")
        except Exception as e:
            logging.warning(f"Error processing value for {key_path}: {e}")
